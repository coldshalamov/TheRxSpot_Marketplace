...
The Rx Spot Platform is the central engine, but it operates within a broader ecosystem of assets:

1.  **The Platform (This Repo):** The operational backbone (Engine).
2.  **Marketing Front Site:** External B2B site driving leads (Separate Repo).
3.  **Template Library:** External collection of design assets/configs (Separate Folder/Repo).

### Template Injection Model

The **Multi-Tenant Storefront** does not hardcode designs. Instead, it acts as a "Renderer" that consumes:
1.  **Configuration:** From the Database (Colors, Logos, Taglines).
2.  **Structure/Layout:** From the **Template Library**.

*Note: Currently, the templates are implemented as dynamic layouts within the Next.js app, but conceptually they represent a distinct library of "white-label skins" that can be applied to tenants.*
...
```
┌─────────────────────────┐       ┌──────────────────────────────────────────┐
│  Template Library       │       │         Customer Storefronts             │
│ (Design Patterns/Assets)│──────▶│ (patients.rx1.com, patients.rx2.com)     │
└─────────────────────────┘       └────────────────────┬─────────────────────┘
                                                       │
                                                       ▼
┌────────────────────────────────────────────────────────────────────────────┐
│                       Next.js Storefront (The Renderer)                     │
│                   - Tenant Resolution (Middleware)                          │
│                   - Dynamic Branding Injection                              │
│                   - Consult-Gating Logic                                    │
└──────────────────────────────────────────────────────┬─────────────────────┘
                                                       │
                                                       ▼ API Requests
┌──────────────────────────────────────────────────────▼─────────────────────┐
│                         Medusa Backend (The Engine)                         │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │  Middleware Stack: Auth • Rate Limit • Tenant Isolation • Audit Log   │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                          │                                                  │
│  ┌───────────────────────▼───────────────────────┐ ┌─────────────────────┐  │
│  │  Custom Modules (Business Logic)              │ │ Medusa Core         │  │
│  │  ┌──────────┐ ┌──────────┐ ┌───────────────┐  │ │ - Products          │  │
│  │  │Business  │ │Consult   │ │Financials     │  │ │ - Orders            │  │
│  │  └──────────┘ └──────────┘ └───────────────┘  │ │ - Carts             │  │
│  └───────────────────────────────────────────────┘ └─────────────────────┘  │
└──────────────────────────────────────────────────────┬──────────────────────┘
                                                       │
...
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────┐  │
│  │PostgreSQL│  │  Redis   │  │   S3     │  │  Stripe  │  │ Provider API │  │
│  │(Primary) │  │ (Cache)  │  │(Documents│  │(Payments)│  │ (Clinical)   │  │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘  └──────────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
Middleware resolves:
1. Extract business_id from URL path OR hostname
2. Load Business record from database
3. Attach to request context
4. **Load Layout/Branding Config** (The "Skin")
...
```typescript
{
  ...
  // Branding (The Template Config)
  logo_url: string
  brand_colors: object      // { primary: "#...", secondary: "#..." }
  template_id: string       // ID of the Template from the library
