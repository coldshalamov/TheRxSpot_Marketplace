...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  await consultationService.restoreConsultations(id)
  const restored = await consultationService.getConsultationOrThrow(id).catch(() => null)
  ...
  return res.json({ consultation: restored ?? existing })
}
