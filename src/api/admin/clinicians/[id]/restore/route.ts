...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const existing = clinicians?.[0]
  if (!existing || (existing.business_id && existing.business_id !== tenantContext.business_id)) {
    if (existing) {
  ...
  await consultationService.restoreClinicians(id)
  const restored = await consultationService.getClinicianOrThrow(id).catch(() => null)
  ...
  return res.json({ clinician: restored ?? existing })
}
