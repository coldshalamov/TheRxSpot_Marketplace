...
export const GET = [
  ...
  async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
      const clinician = await consultationService.retrieveClinician(id)
...
export const PUT = [
  ...
  async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    try {
      ...
      const existingClinician = await consultationService.retrieveClinician(id)
      ...
      const clinician = await consultationService.updateClinician(id, req.body)
      res.json({ clinician })
    } catch (error) {
      if (error instanceof Error && error.message.includes("not found")) {
        return res.status(404).json({ message: error.message })
