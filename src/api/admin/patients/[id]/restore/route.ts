...
export async function POST(req: MedusaRequest, res: MedusaResponse) {
  ...
  const [patients] = await consultationService.listAndCountPatients(
    { id },
  ...
  await consultationService.restorePatients(id)
  const restored = await consultationService.getPatientOrThrow(id).catch(() => null)
  ...
  return res.json({ patient: restored ?? existing })
}
